...
pygame.display.set_caption("PONG_AI")
...
Stext = font.render('P0N6', True, white, black) 
...
while run: 
    ...
    if state == RUNNING:
        ...
        if show_info != True:
            ...
            text = font.render(f'Y: {round(Cy)}  mag: {round(AJ,2),round(vel,2)}', True, white, black) 
            textRect = text.get_rect()  
            textRect.center = (225, 25) 
            text2 = font.render(f'Vectors: {vectors}', True, white, black) 
            textRect2 = text.get_rect()  
            textRect2.center = (675, 25) 
        ...
        if show_info == True:
            pygame.draw.line(win, (255,0,0), (Pcenter2[0],Pcenter2[1]), (Cx,Cy), 5)
